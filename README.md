...
本项目采用多层参数传递机制，确保用户界面参数能够正确传递到渲染器。以下以 `is_2d`（2.5d模式）参数为例，展示完整的参数传递路径：
...
│  1. 用户界面层（side_bar.py）                                                │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  is_2d = st.checkbox("2.5d模式", ...)                            │     │
│     │  return {"is_2d": is_2d, ...}                                    │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                    ↓                                        │
│  2. 应用入口层（app.py / app_pdf.py）                                        │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  render_params = setup_render_params_sidebar()                   │     │
│     │  UIParams(is_2d=render_params["is_2d"], ...)                     │     │
│     │  # 或直接传递:                                                    │     │
│     │  renderer.render_3d_cover(..., is_2d=render_params["is_2d"])    │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                    ↓                                        │
│  3. 数据类层（params.py）                                                    │
│     ┌─────────────────────────────────────────────────────────────────┐     │
...
│  4. 处理器层（processor.py）                                                 │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  render_params = RenderParams(..., is_2d=ui_params.is_2d)        │     │
│     │  renderer.render_3d_cover(..., is_2d=render_params.is_2d)        │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                    ↓                                        │
│  5. 渲染器层（renderer.py）                                                  │
...
│     │      def __init__(self):                                          │     │
│     │          self._is_2d = False  # 类实例变量初始化                   │     │
│     │                                                                   │     │
│     │      def render_3d_cover(self, ..., is_2d=False):                 │     │
│     │          self._is_2d = is_2d  # 接收参数并赋值给实例变量            │     │
│     │                                                                   │     │
│     │      def _calculate_transform_params(self, ...):                  │     │
│     │          if self._is_2d:  # 在任意方法中访问                       │     │
│     │              # 2.5d模式逻辑                                        │     │
│     │          else:                                                    │     │
│     │              # 3d模式逻辑                                          │     │
│     └─────────────────────────────────────────────────────────────────┘     │
...
1. **实例变量存储**：`is_2d` 在 `BookCoverRenderer` 类中作为私有实例变量 `self._is_2d` 存储，只需在 `render_3d_cover()` 方法中设置一次，即可在类的所有方法中访问。

2. **多层传递**：参数从 UI 层逐层传递到渲染器，每一层都有明确的职责：
   - `side_bar.py`：收集用户输入
   - `app.py` / `app_pdf.py`：组装参数
   - `params.py`：数据封装
   - `processor.py`：参数协调
   - `renderer.py`：参数使用

3. **默认值处理**：每个层级都设置了合理的默认值（`False`），确保即使参数未传递也不会出错。

### 添加新参数的步骤

如需添加新的渲染参数，请按以下步骤操作：

1. **side_bar.py**：添加 UI 控件并在返回字典中包含该参数
2. **params.py**：在 `UIParams` 和 `RenderParams` 中添加字段
3. **app.py** / **app_pdf.py**：在创建 `UIParams` 或调用 `render_3d_cover()` 时传递参数
4. **processor.py**：在 `RenderParams` 创建和 `render_3d_cover()` 调用时传递参数
5. **renderer.py**：
   - 在 `__init__()` 中初始化实例变量
   - 在 `render_3d_cover()` 中接收参数并赋值给实例变量
   - 在需要的方法中使用 `self._参数名` 访问
