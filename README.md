...
本项目采用多层参数传递机制，确保用户界面参数能够正确传递到渲染器。以下以 `spine_width_ratio`（书脊拉伸）参数为例，展示完整的参数传递路径：
...
│  1. 用户界面层（side_bar.py）                                                │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  spine_width_ratio = st.slider("书脊拉伸", ...)                  │     │
│     │  return {"spine_width_ratio": spine_width_ratio, ...}            │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                    ↓                                        │
│  2. 应用入口层（app.py / app_pdf.py）                                        │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  render_params = setup_render_params_sidebar()                   │     │
│     │  UIParams(spine_width_ratio=render_params["spine_width_ratio"]) │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                    ↓                                        │
│  3. 数据类层（params.py）                                                    │
│     ┌─────────────────────────────────────────────────────────────────┐     │
...
│  4. 处理器层（processor.py）                                                 │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  render_params = RenderParams(..., spine_width_ratio=...)        │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                    ↓                                        │
│  5. 渲染器层（renderer.py）                                                  │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │  class BookCoverRenderer:                                         │     │
│     │      def render_3d_cover(self, ...):                              │     │
│     │          # 使用参数进行渲染                                       │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
1. **多层传递**：参数从 UI 层逐层传递到渲染器，每一层都有明确的职责：
   - `side_bar.py`：收集用户输入
...
2. **默认值处理**：每个层级都设置了合理的默认值，确保即使参数未传递也不会出错。
...
5. **renderer.py**：在 `render_3d_cover()` 中接收参数并使用
