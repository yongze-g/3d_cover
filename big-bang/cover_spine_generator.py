...
def split_image_by_symmetry(img_path, symmetry_positions, output_dir):
    """
    根据对称位置将图片分割成若干块
    ...
    # 收集所有分割边界
    # horizontal扫描结果：X坐标分割线（垂直分割线）
    x_lines = []
    # vertical扫描结果：Y坐标分割线（水平分割线）
    y_lines = []
    ...
    # 添加图片边界作为起始和结束点
    # X坐标边界
    x_coords = [0] + sorted(x_lines) + [width]
    # Y坐标边界  
    y_coords = [0] + sorted(y_lines) + [height]
            ...
            for offset in range(height // 2):
                ...
                if intersection and len(intersection) >= 6:
                    ...
                    for x in processed_positions:
                        ...
                        # 添加编号
                        draw.text((x+5, (top_row + bottom_row) // 2), str(line_counter), fill="yellow")
            ...
            for offset in range(width // 2):
                ...
                if intersection and len(intersection) >= 4:
                    ...
                    for y in final_positions:
                        ...
                        # 添加编号
                        draw.text(((left_col + right_col) // 2, y-15), str(line_counter), fill="yellow")
...
def process_image_for_cover_and_spine(img_path, output_dir):
    ...
    # 2. 根据对称位置分割图片
    split_images = split_image_by_symmetry(img_path, symmetry_positions, output_dir)
